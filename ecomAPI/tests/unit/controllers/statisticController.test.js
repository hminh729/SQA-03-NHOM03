...
describe("=== STATISTIC CONTROLLER TEST SUITE ===", () => {
  ...
  describe("TEST SUITE 1: getCountCardStatistic() Function", () => {
    test("TC_001: Should successfully get count card statistic", async () => {
      ...
      statisticService.getCountCardStatistic.mockResolvedValue(mockServiceResponse);

      await statisticController.getCountCardStatistic(mockRequest, mockResponse);

      expect(statisticService.getCountCardStatistic).toHaveBeenCalledWith(mockQuery);
    ...
    test("TC_002: Should handle error in getCountCardStatistic", async () => {
      statisticService.getCountCardStatistic.mockRejectedValue(new Error("Service Error"));
  ...
  describe("TEST SUITE 2: getCountStatusOrder() Function", () => {
    test("TC_003: Should successfully get count status order", async () => {
      ...
      statisticService.getCountStatusOrder.mockResolvedValue(mockServiceResponse);

      await statisticController.getCountStatusOrder(mockRequest, mockResponse);

      expect(statisticService.getCountStatusOrder).toHaveBeenCalledWith(mockQuery);
      expect(mockResponse.status).toHaveBeenCalledWith(200);
      expect(mockResponse.json).toHaveBeenCalledWith(mockServiceResponse);
    });

    test("TC_004: Should handle error in getCountStatusOrder", async () => {
      statisticService.getCountStatusOrder.mockRejectedValue(new Error("Service Error"));
  ...
  describe("TEST SUITE 3: getStatisticByMonth() Function", () => {
    test("TC_005: Should successfully get statistic by month", async () => {
      ...
      statisticService.getStatisticByMonth.mockResolvedValue(mockServiceResponse);

      await statisticController.getStatisticByMonth(mockRequest, mockResponse);

      expect(statisticService.getStatisticByMonth).toHaveBeenCalledWith(mockQuery);
      expect(mockResponse.status).toHaveBeenCalledWith(200);
      expect(mockResponse.json).toHaveBeenCalledWith(mockServiceResponse);
    });

    test("TC_006: Should handle error in getStatisticByMonth", async () => {
      statisticService.getStatisticByMonth.mockRejectedValue(new Error("Service Error"));
  ...
  describe("TEST SUITE 4: getStatisticByDay() Function", () => {
    test("TC_007: Should successfully get statistic by day", async () => {
      ...
      expect(statisticService.getStatisticByDay).toHaveBeenCalledWith(mockQuery);
      expect(mockResponse.status).toHaveBeenCalledWith(200);
      expect(mockResponse.json).toHaveBeenCalledWith(mockServiceResponse);
    });

    test("TC_008: Should handle error in getStatisticByDay", async () => {
      statisticService.getStatisticByDay.mockRejectedValue(new Error("Service Error"));
  ...
  describe("TEST SUITE 5: getStatisticOverturn() Function", () => {
    test("TC_009: Should successfully get statistic overturn", async () => {
      ...
      statisticService.getStatisticOverturn.mockResolvedValue(mockServiceResponse);

      await statisticController.getStatisticOverturn(mockRequest, mockResponse);

      expect(statisticService.getStatisticOverturn).toHaveBeenCalledWith(mockQuery);
      expect(mockResponse.status).toHaveBeenCalledWith(200);
      expect(mockResponse.json).toHaveBeenCalledWith(mockServiceResponse);
    });

    test("TC_010: Should handle error in getStatisticOverturn", async () => {
      statisticService.getStatisticOverturn.mockRejectedValue(new Error("Service Error"));
  ...
  describe("TEST SUITE 6: getStatisticProfit() Function", () => {
    test("TC_011: Should successfully get statistic profit", async () => {
      ...
      statisticService.getStatisticProfit.mockResolvedValue(mockServiceResponse);

      await statisticController.getStatisticProfit(mockRequest, mockResponse);

      expect(statisticService.getStatisticProfit).toHaveBeenCalledWith(mockQuery);
      expect(mockResponse.status).toHaveBeenCalledWith(200);
      expect(mockResponse.json).toHaveBeenCalledWith(mockServiceResponse);
    });

    test("TC_012: Should handle error in getStatisticProfit", async () => {
      statisticService.getStatisticProfit.mockRejectedValue(new Error("Service Error"));
  ...
  describe("TEST SUITE 7: getStatisticStockProduct() Function", () => {
    test("TC_013: Should successfully get statistic stock product", async () => {
      ...
      statisticService.getStatisticStockProduct.mockResolvedValue(mockServiceResponse);

      await statisticController.getStatisticStockProduct(mockRequest, mockResponse);

      expect(statisticService.getStatisticStockProduct).toHaveBeenCalledWith(mockQuery);
      expect(mockResponse.status).toHaveBeenCalledWith(200);
      expect(mockResponse.json).toHaveBeenCalledWith(mockServiceResponse);
    });

    test("TC_014: Should handle error in getStatisticStockProduct", async () => {
      statisticService.getStatisticStockProduct.mockRejectedValue(new Error("Service Error"));
